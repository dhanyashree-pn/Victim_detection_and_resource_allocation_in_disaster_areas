...
#Use the below function when classifying image and video file after making changes in the uploader of main function
# when you want to try using video as the input file. Sample input video is available in video folder.


# def classify_injury(file_path):
#     file_type = classify_file_type(file_path)
#
#     if file_type == 'video':
#         video_path = file_path
#         statement,output,temperatures=only_face(video_path)
#         output_classification=output
#
#     elif file_type == 'image':
#         image_path = file_path
#         image = cv2.imread(image_path)
#         processed_image, injury_label,temperatures = process_frame(image)
#         print(injury_label)
#         if injury_label == "Not Injured":
#             statement = "No severely injured person detected."
#         else:
#             statement = "Severely injured person detected."
#         print(statement)
#         output_classification=processed_image
#     else:
#         statement=f"{file_path} is neither a video nor an image file."
#
#     return statement,output_classification,temperatures


def classify_injury(file_path,flag):
    image_path = file_path
    image = cv2.imread(image_path)


    processed_image, injury_label,temperatures = process_frame(image,flag)
    print(injury_label)
    if injury_label == "Not Injured":
        statement = "No severely injured person detected."
    else:
